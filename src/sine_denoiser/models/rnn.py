...
class RNN(DenoiserModel):
    ...
    The context window is treated as a length-``W`` sequence. At each timestep
    the model consumes ``[x_ctx[t]] ++ one_hot(c, num_components)``; the
    selector is broadcast to every step. A linear projection maps each
    per-timestep hidden state to a scalar, giving an output of shape
    ``(B, W)``.
    """
    ...
    def forward(self, x_ctx: Tensor, c: Tensor) -> Tensor:
        ...
        x_seq = x_ctx.unsqueeze(-1)
        seq = torch.cat([x_seq, one_hot_seq], dim=-1)
        h_seq, _ = self.rnn(seq)
        return self.proj(h_seq).squeeze(-1)
